<template>
  <main class="flex min-h-screen items-center bg-indigo-950">
    <RouterView v-slot="{ Component }" :key="route.fullPath" class="mx-auto w-full max-w-2xl px-6 pb-24 pt-10 lg:px-8">
      <template v-if="Component">
        <Transition mode="out-in">
          <Suspense>
            ...
            <!-- loading state -->
            <template #fallback>
              <div class="mx-auto flex min-h-screen max-w-2xl items-center justify-center p-6 lg:px-8">
                <div role="status">
  ...
  <footer class="fixed bottom-0 right-0 p-4">
    <RouterLink :to="{ name: 'home' }" target="_blank" class="-m-1.5 p-1.5 font-bold text-white">
      <span class="text-xs font-light">Powered By </span>Form<span class="text-indigo-400">Thing</span>
    </RouterLink>
  </footer>
</template>
